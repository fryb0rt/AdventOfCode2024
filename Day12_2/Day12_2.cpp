		...
		while (!stack.empty()) {
			...
			if (p.x == 0 || lines[p.y][p.x - 1] != '-') {
				if (p.y == 0 || lines[p.y - 1][p.x] != '-' ||
					(p.x > 0 && lines[p.y - 1][p.x - 1] == '-')) { // Nothing above
					++perimeter;
				}
			}
			if (p.y == 0 || lines[p.y - 1][p.x] != '-') {
				if (p.x == 0 || lines[p.y][p.x - 1] != '-' || (p.y > 0 && lines[p.y - 1][p.x - 1] == '-')) { // Nothing to the left
					++perimeter;
				}
			}
			if (p.x == lines[0].size() - 1 || lines[p.y][p.x + 1] != '-') {
				if (p.y == 0 || lines[p.y - 1][p.x] != '-' ||
					(p.x < lines[0].size() - 1 && lines[p.y - 1][p.x + 1] == '-')) { // Nothing above
					++perimeter;
				}
			}
			if (p.y == lines.size() - 1 || lines[p.y + 1][p.x] != '-') {
				if (p.x == 0 || lines[p.y][p.x - 1] != '-' ||
					(p.y < lines.size() - 1 && lines[p.y + 1][p.x - 1] == '-')) { // Nothing to the left
					++perimeter;
				}
			}
		}
